...
public class MultiMarkerObject : MonoBehaviour
{
    ...
    public MarkerDetectorVR markerDetectorVR;
    ...
    void Update()
    {
        ...
        } else if(markerDetectorVR != null)
        {
            foreach (MultiMarker marker in markers)
            {
                if (markerDetectorVR.GetTransformationOfMarker(marker.id, out rotation, out translation))
                {
                    Vector3 tmptranslation = rotation * marker.offset;
                    Vector4 tmporientation = QuatToVec4(rotation);
                    if (Vector4.Dot(tmporientation, meanRotation) >= 0)
            ...
            if (found)
            {
                ...
                rotation.Normalize();
                translation = meanTranslation / div;
                markerDetectorVR.ApplyProjectionAndViewInverse(ref rotation, ref translation);
                if (!active)
                {
                    active = true;
                    gameObject.transform.localScale = oldScale;
